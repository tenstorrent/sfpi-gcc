...
#define WORMHOLE_BUILTIN(id, fmt, en, cc, lv, hho, dap, mp) { riscv_sfpu_insn_data::id, #id, nullptr, cc, lv, hho, dap, mp },
#define WORMHOLE_NO_TGT_BUILTIN(id, fmt, en, cc, lv, hho, dap, mp) { riscv_sfpu_insn_data::id, #id, nullptr, cc, lv, hho, dap, mp },
#include "sfpu-insn.h"
...
void riscv_sfpu_emit_sfpscmp_ex(rtx addr, rtx v, rtx f, rtx mod)
{
  ...
  rtx setcc_mod = GEN_INT(cmp_ex_to_setcc_mod1_map[cmp]);
  if (need_sub) {
  ...
  if (cmp == SFPCMP_EX_MOD1_CC_LTE || cmp == SFPCMP_EX_MOD1_CC_GT) {
    emit_insn(gen_riscv_sfpsetcc_v(v, GEN_INT(SFPSETCC_MOD1_LREG_GTE0)));
    emit_insn(gen_riscv_sfpsetcc_v(v, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPCMP_EX_MOD1_CC_LTE) {
      emit_insn(gen_riscv_sfpcompc());
    }
  } else {
...
void riscv_sfpu_emit_sfpvcmp_ex(rtx v1, rtx v2, rtx mod)
{
  ...
  unsigned int cmp = INTVAL(mod) & SFPCMP_EX_MOD1_CC_MASK;
  if (cmp == SFPCMP_EX_MOD1_CC_LTE || cmp == SFPCMP_EX_MOD1_CC_GT) {
    emit_insn(gen_riscv_sfpsetcc_v(tmp, GEN_INT(SFPSETCC_MOD1_LREG_GTE0)));
    emit_insn(gen_riscv_sfpsetcc_v(tmp, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPCMP_EX_MOD1_CC_LTE) {
      emit_insn(gen_riscv_sfpcompc());
    }
  } else {
...
void riscv_wormhole_sfpu_emit_sfpassignlr(rtx dst, rtx lr)
{
  int lregnum = INTVAL(lr);
  SET_REGNO(dst, SFPU_REG_FIRST + lregnum);
  emit_insn(gen_riscv_wormhole_sfpassignlr_int(dst));
}

void riscv_wormhole_sfpu_emit_nonimm_dst(rtx buf_addr, rtx dst, int nnops, rtx dst_lv, rtx imm,
				int base, int lshft, int rshft, int dst_shft)
{
    rtx insn = gen_reg_rtx(SImode);
    emit_insn(gen_ashlsi3(insn, imm, GEN_INT(lshft)));
    emit_insn(gen_lshrsi3(insn, insn, GEN_INT(rshft)));
    emit_insn(gen_riscv_wormhole_sfpnonimm_dst(dst, buf_addr, GEN_INT(nnops), dst_lv, GEN_INT(base), GEN_INT(dst_shft), insn));
}

void riscv_wormhole_sfpu_emit_nonimm_dst_src(rtx buf_addr, rtx dst, int nnops, rtx dst_lv, rtx src, rtx imm, int base, int lshft, int rshft, int dst_shft, int src_shft)
{
    rtx insn = gen_reg_rtx(SImode);
    emit_insn(gen_ashlsi3(insn, imm, GEN_INT(lshft)));
    emit_insn(gen_lshrsi3(insn, insn, GEN_INT(rshft)));
    emit_insn(gen_riscv_wormhole_sfpnonimm_dst_src(dst, buf_addr, GEN_INT(nnops), dst_lv, src, GEN_INT(base), GEN_INT(dst_shft), GEN_INT(src_shft), insn));
}

void riscv_wormhole_sfpu_emit_nonimm_src(rtx buf_addr, rtx src, int nnops, rtx imm, int base, int lshft, int rshft, int src_shft)
{
    rtx insn = gen_reg_rtx(SImode);
    emit_insn(gen_ashlsi3(insn, imm, GEN_INT(lshft)));
    emit_insn(gen_lshrsi3(insn, insn, GEN_INT(rshft)));
    emit_insn(gen_riscv_wormhole_sfpnonimm_src(src, buf_addr, GEN_INT(nnops), GEN_INT(base), GEN_INT(src_shft), insn));
}

void riscv_wormhole_sfpu_emit_nonimm_store(rtx buf_addr, rtx src, int nnops, rtx imm, int base, int lshft, int rshft, int src_shft)
{
    // This is exactly like _src, exists so peephole can handle the store-to-load nop
    rtx insn = gen_reg_rtx(SImode);
    emit_insn(gen_ashlsi3(insn, imm, GEN_INT(lshft)));
    emit_insn(gen_lshrsi3(insn, insn, GEN_INT(rshft)));
    emit_insn(gen_riscv_wormhole_sfpnonimm_store(src, buf_addr, GEN_INT(nnops), GEN_INT(base), GEN_INT(src_shft), insn));
}

char const * riscv_wormhole_sfpu_output_nonimm_store_and_nops(const char *sw, int nnops, rtx operands[])
{
  char const *out = sw;
  while (nnops-- > 0) {
     output_asm_insn(out, operands);
...
void riscv_wormhole_sfpu_emit_sfpload(rtx dst, rtx lv, rtx addr, rtx mod, rtx mode, rtx imm)
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_riscv_wormhole_sfpload_int(dst, lv, mod, mode, imm));
  } else {
    int base = TT_OP_SFPLOAD(0, INTVAL(mod), 0);
    riscv_wormhole_sfpu_emit_nonimm_dst(addr, dst, 0, lv, imm, base, 16, 16, 20);
  }
}

void riscv_wormhole_sfpu_emit_sfploadi(rtx dst, rtx lv, rtx addr, rtx mod, rtx imm)
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_riscv_wormhole_sfploadi_int(dst, lv, mod, imm));
  } else {
    int base = TT_OP_SFPLOADI(0, INTVAL(mod), 0);
    riscv_wormhole_sfpu_emit_nonimm_dst(addr, dst, 0, lv, imm, base, 16, 16, 20);
  }
...
void riscv_wormhole_sfpu_emit_sfpiadd_i(rtx dst, rtx lv, rtx addr, rtx src, rtx imm, rtx mod)
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_riscv_wormhole_sfpiadd_i_int(dst, lv, src, riscv_sfpu_clamp_signed(imm, 0x7FF), mod));
  } else {
    int mod1 = INTVAL(mod);
    int base = TT_OP_SFPIADD(0, 0, 0, mod1);
    int nnops = (mod1 < 3 || mod1 > 7) ? 3 : 2;
    riscv_wormhole_sfpu_emit_nonimm_dst_src(addr, dst, nnops, lv, src, imm, base, 20, 8, 4, 8);
  }
}
...
// handle it and if it did, the result would be inefficient.
//
void riscv_wormhole_sfpu_emit_sfpiadd_i_ex(rtx dst, rtx lv, rtx addr, rtx src, rtx imm, rtx mod)
{
  unsigned int modi = INTVAL(mod);
  unsigned int cmp = modi & SFPCMP_EX_MOD1_CC_MASK;
  unsigned int base_mod = modi & ~SFPCMP_EX_MOD1_CC_MASK;
  ...
  // Decompose aggregate comparisons, recurse
  if (cmp == SFPCMP_EX_MOD1_CC_LTE || cmp == SFPCMP_EX_MOD1_CC_GT) {
    riscv_wormhole_sfpu_emit_sfpiadd_i_ex(dst, lv, addr, src, imm, GEN_INT(base_mod | SFPCMP_EX_MOD1_CC_GTE));
    riscv_wormhole_sfpu_emit_sfpiadd_i_ex(dst, lv, addr, dst, GEN_INT(0), GEN_INT(base_mod | SFPCMP_EX_MOD1_CC_NE));
    if (cmp == SFPCMP_EX_MOD1_CC_LTE) {
      emit_insn(gen_riscv_wormhole_sfpcompc());
    }
    return;
  }
  ...
  if (need_loadi) {
    // Load imm into dst
    int loadi_mod = is_signed ? SFPLOADI_MOD0_SHORT : SFPLOADI_MOD0_USHORT;
    riscv_wormhole_sfpu_emit_sfploadi(dst, riscv_sfpu_gen_const0_vector(), addr, GEN_INT(loadi_mod), imm);

    unsigned int mod1 = is_sub ? SFPIADD_MOD1_ARG_2SCOMP_LREG_DST : SFPIADD_MOD1_ARG_LREG_DST;
    if (cmp == SFPCMP_EX_MOD1_CC_LT || cmp == SFPCMP_EX_MOD1_CC_GTE) {
      // Perform op w/ compare
      mod1 |= (cmp == SFPCMP_EX_MOD1_CC_LT) ? SFPIADD_MOD1_CC_LT0 : SFPIADD_MOD1_CC_GTE0;
      emit_insn(gen_riscv_wormhole_sfpiadd_v(dst, dst, src, GEN_INT(mod1)));
      need_setcc = false;
    } else {
      // Perform op w/o compare, compare with SETCC
      mod1 |= SFPIADD_MOD1_CC_NONE;
	...
	// Perform op w/ compare
	unsigned int mod1 = (cmp == SFPCMP_EX_MOD1_CC_LT) ? SFPIADD_MOD1_CC_LT0 : SFPIADD_MOD1_CC_GTE0;
	emit_insn(gen_riscv_wormhole_sfpiadd_i_int(dst, lv, src, imm, GEN_INT(mod1 | SFPIADD_MOD1_ARG_IMM)));
	need_setcc = false;
      } else {
	// Perform op w/o compare
	emit_insn(gen_riscv_wormhole_sfpiadd_i_int(dst, lv, src, imm,
					  GEN_INT(SFPIADD_MOD1_ARG_IMM | SFPIADD_MOD1_CC_NONE)));
	set_cc_arg = dst;
      }
    } else if ((cmp & SFPCMP_EX_MOD1_CC_MASK) == 0) {
    ...
    riscv_wormhole_sfpu_emit_sfpiadd_i(dst, lv, addr, src, imm, GEN_INT(mod1));
  }

  if (need_setcc) {
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(set_cc_arg, GEN_INT(cmp_ex_to_setcc_mod1_map[cmp])));
  }
}

// See comment block above sfpiadd_i_ex
void riscv_wormhole_sfpu_emit_sfpiadd_v_ex(rtx dst, rtx srcb, rtx srca, rtx mod)
{
  unsigned int modi = INTVAL(mod);
  unsigned int cmp = modi & SFPCMP_EX_MOD1_CC_MASK;
  unsigned int base_mod = modi & ~SFPCMP_EX_MOD1_CC_MASK;
  ...
  // Decompose aggregate comparisons, recurse
  if (cmp == SFPCMP_EX_MOD1_CC_LTE || cmp == SFPCMP_EX_MOD1_CC_GT) {
    riscv_wormhole_sfpu_emit_sfpiadd_v_ex(dst, srcb, srca, GEN_INT(base_mod | SFPCMP_EX_MOD1_CC_GTE));
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(dst, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPCMP_EX_MOD1_CC_LTE) {
      emit_insn(gen_riscv_wormhole_sfpcompc());
    }
    return;
  ...
  if (cmp == SFPCMP_EX_MOD1_CC_LT || cmp == SFPCMP_EX_MOD1_CC_GTE) {
    // Perform op w/ compare
    mod1 |= (cmp == SFPCMP_EX_MOD1_CC_LT) ? SFPIADD_MOD1_CC_LT0 : SFPIADD_MOD1_CC_GTE0;
    emit_insn(gen_riscv_wormhole_sfpiadd_v(dst, srcb, srca, GEN_INT(mod1)));
  } else {
    // Perform op w/o compare
    mod1 |= SFPIADD_MOD1_CC_NONE;
    emit_insn(gen_riscv_wormhole_sfpiadd_v(dst, srcb, srca, GEN_INT(mod1)));
    if (cmp != 0) {
      // Must be EQ0 or NE0, compare with SETCC
      emit_insn(gen_riscv_wormhole_sfpsetcc_v(dst, GEN_INT(cmp_ex_to_setcc_mod1_map[cmp])));
    }
  }
}

void riscv_wormhole_sfpu_emit_sfpscmp_ex(rtx addr, rtx v, rtx f, rtx mod)
{
  bool need_sub = false;
  rtx ref_val = gen_reg_rtx(V64SFmode);
  ...
  if (GET_CODE(f) == CONST_INT) {
	...
	// Could add more CReg values here, doubt they show up in cmp
      case 0x3f80:
	riscv_wormhole_sfpu_emit_sfpassignlr(ref_val, GEN_INT(CREG_IDX_1));
	break;
      case 0xbf00:
	riscv_wormhole_sfpu_emit_sfpassignlr(ref_val, GEN_INT(CREG_IDX_NEG_0P5));
	break;
      case 0xbf80:
	riscv_wormhole_sfpu_emit_sfpassignlr(ref_val, GEN_INT(CREG_IDX_NEG_1));
	break;
      default:
	int loadi_mod = ((INTVAL(mod) & SFPSCMP_EX_MOD1_FMT_A) == SFPSCMP_EX_MOD1_FMT_A) ?
	    SFPLOADI_MOD0_FLOATA : SFPLOADI_MOD0_FLOATB;
	riscv_wormhole_sfpu_emit_sfploadi(ref_val, riscv_sfpu_gen_const0_vector(), addr, GEN_INT(loadi_mod), f);
	break;
      }
    }
  } else {
  ...
  if (need_sub) {
    rtx neg_one = gen_reg_rtx(V64SFmode);
    rtx tmp = gen_reg_rtx(V64SFmode);
    riscv_wormhole_sfpu_emit_sfpassignlr(neg_one, GEN_INT(CREG_IDX_NEG_1));
    emit_insn(gen_riscv_wormhole_sfpmad(tmp, ref_val, neg_one, v, GEN_INT(0)));
    v = tmp;
  }

  if (cmp == SFPCMP_EX_MOD1_CC_LTE || cmp == SFPCMP_EX_MOD1_CC_GT) {
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(v, GEN_INT(SFPSETCC_MOD1_LREG_GTE0)));
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(v, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPCMP_EX_MOD1_CC_LTE) {
      emit_insn(gen_riscv_wormhole_sfpcompc());
    }
  } else {
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(v, setcc_mod));
  }
}

// Compare two vectors by subtracting v2 from v1 and doing a setcc
void riscv_wormhole_sfpu_emit_sfpvcmp_ex(rtx v1, rtx v2, rtx mod)
{
  rtx tmp = gen_reg_rtx(V64SFmode);
  rtx neg1 = gen_reg_rtx(V64SFmode);

  riscv_wormhole_sfpu_emit_sfpassignlr(neg1, GEN_INT(CREG_IDX_NEG_1));
  emit_insn(gen_riscv_wormhole_sfpmad(tmp, v2, neg1, v1, GEN_INT(0)));

  unsigned int cmp = INTVAL(mod) & SFPCMP_EX_MOD1_CC_MASK;
  if (cmp == SFPCMP_EX_MOD1_CC_LTE || cmp == SFPCMP_EX_MOD1_CC_GT) {
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(tmp, GEN_INT(SFPSETCC_MOD1_LREG_GTE0)));
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(tmp, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPCMP_EX_MOD1_CC_LTE) {
      emit_insn(gen_riscv_wormhole_sfpcompc());
    }
  } else {
    emit_insn(gen_riscv_wormhole_sfpsetcc_v(tmp, GEN_INT(cmp_ex_to_setcc_mod1_map[INTVAL(mod)])));
  }
}

void riscv_wormhole_sfpu_emit_sfpdivp2(rtx dst, rtx lv, rtx addr, rtx imm, rtx src, rtx mod)
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_riscv_wormhole_sfpdivp2_int(dst, lv, riscv_sfpu_clamp_signed(imm, 0x7FF), src, mod));
  } else {
    int base = TT_OP_SFPDIVP2(0, 0, 0, INTVAL(mod));
    riscv_wormhole_sfpu_emit_nonimm_dst_src(addr, dst, 2, lv, src, imm, base, 20, 8, 4, 8);
  }
}
