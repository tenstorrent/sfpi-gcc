...
void rvtt_bh_emit_sfpxloadi(rtx dst, rtx lv, rtx addr, rtx mod, rtx imm, rtx nonimm, rtx id)
{
  ...
  if (int_mod & SFPXLOADI_MOD0_32BIT_MASK) {
    ...
    if (GET_CODE(imm) == CONST_INT) {
      emit_insn(gen_rvtt_bh_sfploadi_int(dst, lv, GEN_INT(int_mod), rvtt_clamp_signed(imm, 0x7FFF)));
    } else {
...
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_rvtt_bh_sfpiadd_i_int(dst, lv, src, rvtt_clamp_signed(imm, 0x7FF), mod));
  } else {
    unsigned int mod1 = UINTVAL(mod);
    unsigned long int op = TT_OP_BH_SFPIADD(0, 0, 0, mod1);
...
//
void rvtt_bh_emit_sfpxiadd_i(rtx dst, rtx lv, rtx addr, rtx src, rtx imm, rtx mod)
{
  ...
  if (cmp == SFPXCMP_MOD1_CC_LTE || cmp == SFPXCMP_MOD1_CC_GT) {
    rvtt_bh_emit_sfpxiadd_i(dst, lv, addr, src, imm, GEN_INT(base_mod | SFPXCMP_MOD1_CC_GTE));
    rvtt_bh_emit_sfpxiadd_i(dst, lv, addr, dst, GEN_INT(0), GEN_INT(base_mod | SFPXCMP_MOD1_CC_NE));
    if (cmp == SFPXCMP_MOD1_CC_LTE) {
      emit_insn(gen_rvtt_bh_sfpcompc());
    }
    return;
  }
  ...
  if (need_loadi) {
    ...
    rvtt_bh_emit_sfpxloadi(dst, rvtt_gen_const0_vector(), addr, GEN_INT(loadi_mod), imm, GEN_INT(0), GEN_INT(0));
  ...
  if (need_setcc) {
    emit_insn(gen_rvtt_bh_sfpsetcc_v(set_cc_arg, GEN_INT(rvtt_cmp_ex_to_setcc_mod1_map[cmp])));
  }
}
...
void rvtt_bh_emit_sfpxiadd_v(rtx dst, rtx srcb, rtx srca, rtx mod)
{
  ...
  if (cmp == SFPXCMP_MOD1_CC_LTE || cmp == SFPXCMP_MOD1_CC_GT) {
    ...
    emit_insn(gen_rvtt_bh_sfpsetcc_v(dst, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPXCMP_MOD1_CC_LTE) {
      emit_insn(gen_rvtt_bh_sfpcompc());
    }
    return;
  ...
  if (cmp == SFPXCMP_MOD1_CC_LT || cmp == SFPXCMP_MOD1_CC_GTE) {
    ...
    if (cmp != 0) {
      ...
      emit_insn(gen_rvtt_bh_sfpsetcc_v(dst, GEN_INT(rvtt_cmp_ex_to_setcc_mod1_map[cmp])));
    }
  }
}
...
void rvtt_bh_emit_sfpxfcmps(rtx addr, rtx v, rtx f, rtx mod)
{
  ...
  if (fval != 0 &&
	...
	(fmt != SFPXSCMP_MOD1_FMT_FLOAT && fval == 0x3f80)) {
      ...
      rvtt_bh_emit_sfpxloadi(ref_val, rvtt_gen_const0_vector(), addr,
  ...
  if (cmp == SFPXCMP_MOD1_CC_LTE || cmp == SFPXCMP_MOD1_CC_GT) {
    emit_insn(gen_rvtt_bh_sfpsetcc_v(v, GEN_INT(SFPSETCC_MOD1_LREG_GTE0)));
    emit_insn(gen_rvtt_bh_sfpsetcc_v(v, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPXCMP_MOD1_CC_LTE) {
      emit_insn(gen_rvtt_bh_sfpcompc());
    }
  } else {
    emit_insn(gen_rvtt_bh_sfpsetcc_v(v, setcc_mod));
  }
}
...
void rvtt_bh_emit_sfpxfcmpv(rtx v1, rtx v2, rtx mod)
{
  ...
  unsigned int cmp = INTVAL(mod) & SFPXCMP_MOD1_CC_MASK;
  if (cmp == SFPXCMP_MOD1_CC_LTE || cmp == SFPXCMP_MOD1_CC_GT) {
    emit_insn(gen_rvtt_bh_sfpsetcc_v(tmp, GEN_INT(SFPSETCC_MOD1_LREG_GTE0)));
    emit_insn(gen_rvtt_bh_sfpsetcc_v(tmp, GEN_INT(SFPSETCC_MOD1_LREG_NE0)));
    if (cmp == SFPXCMP_MOD1_CC_LTE) {
      emit_insn(gen_rvtt_bh_sfpcompc());
    }
  } else {
    emit_insn(gen_rvtt_bh_sfpsetcc_v(tmp, GEN_INT(rvtt_cmp_ex_to_setcc_mod1_map[INTVAL(mod)])));
  }
}
...
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_rvtt_bh_sfpdivp2_int(dst, lv, rvtt_clamp_signed(imm, 0x7FF), src, mod));
  } else {
    unsigned long int op = TT_OP_BH_SFPDIVP2(0, 0, 0, UINTVAL(mod));
    emit_insn(gen_rvtt_sfpnonimm_dst_src(dst, addr, GEN_INT(0), lv, src, GEN_INT(4), GEN_INT(8), nonimm, GEN_INT(op), id));
  }
}
...
{
  if (GET_CODE(imm) == CONST_INT) {
    emit_insn(gen_rvtt_bh_sfpstochrnd_i_int(dst, lv, mode, rvtt_clamp_unsigned(imm, 0x1F), src, mod));
  } else {
    unsigned int mod1 = UINTVAL(mod);
    unsigned int imode = UINTVAL(mode);
    unsigned long int op = TT_OP_BH_SFP_STOCH_RND(imode, 0, 0, 0, 0, mod1);
    emit_insn(gen_rvtt_sfpnonimm_dst_src(dst, addr, GEN_INT(0), lv, src, GEN_INT(4), GEN_INT(8), nonimm, GEN_INT(op), id));
  }
}
