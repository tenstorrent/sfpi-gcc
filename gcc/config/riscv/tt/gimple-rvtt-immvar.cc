	      ...
	      tree addr, tree val, tree res = nullptr)
{
  ...
  if (!needs_both)
  ...
  else if (SSA_VAR_P (lower))
    {
      ...
      auto *lower_stmt = gimple_build_assign (lower, BIT_AND_EXPR,
					      val, build_int_cst (unsigned_type_node, 0xffff));
      gimple_set_location (lower_stmt, loc);
      gsi_insert_before (&gsi, lower_stmt, GSI_SAME_STMT);
    }
  else
    lower = build_int_cst (TREE_TYPE (lower), TREE_INT_CST_LOW (lower) & 0xffff);
  ...
  if (needs_both)
      ...
      if (SSA_VAR_P (val))
	{
	  ...
	  auto *shift_stmt = gimple_build_assign (shift, RSHIFT_EXPR,
						  val, build_int_cst (unsigned_type_node, 16));
	  gimple_set_location (shift_stmt, loc);
	  gsi_insert_before (&gsi, shift_stmt, GSI_SAME_STMT);
	}
      else
	shift = build_int_cst (TREE_TYPE (val), TREE_INT_CST_LOW (val) >> 16);

      res = emit_sfploadi (gsi, loc, SFPLOADI_MOD0_UPPER, tmp,
			   addr, shift, res);
    }
  else
    res = tmp;
