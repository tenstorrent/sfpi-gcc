  ...
  if (need_loadi)
    {
      ...
      rvtt_bh_emit_sfpxloadi (dst, rvtt_vec0_rtx, addr, GEN_INT(loadi_mod), imm, const0_rtx, const0_rtx);
...
void rvtt_bh_emit_sfpxfcmps(rtx addr, rtx v, rtx f, rtx mod)
{
  ...
  if (fval != 0 &&
	...
	ref_val = rvtt_gen_rtx_creg (V64SFmode, CREG_IDX_1);
      else if ((fmt == SFPXSCMP_MOD1_FMT_FLOAT && fval == 0xbf800000) ||
		 (fmt != SFPXSCMP_MOD1_FMT_FLOAT && fval == 0xbf80))
	ref_val = rvtt_gen_rtx_creg (V64SFmode, CREG_IDX_NEG_1);
      else
  ...
  if (need_sub) {
    rtx tmp = gen_reg_rtx(V64SFmode);
    rtx neg_one = rvtt_gen_rtx_creg (V64SFmode, CREG_IDX_NEG_1);
...
{
  rtx tmp = gen_reg_rtx(V64SFmode);
  rtx neg1 = rvtt_gen_rtx_creg (V64SFmode, CREG_IDX_NEG_1);
