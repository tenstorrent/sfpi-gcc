...
{
  tree tmp1 = emit_sfpxloadi (SFPLOADI_MOD0_USHORT, insnd, stmt, gsip);
...
expand_complex (gcall *stmt, const rvtt_insn_data *insnd, gimple_stmt_iterator *gsip)
{
  switch (insnd->id)
      ...
      if ((get_int_arg (stmt, insnd->mod_arg ()) & SFPXLOADI_MOD0_32BIT_MASK) != 0)
	{
	  ...
	  gimple_call_set_arg (stmt, insnd->mod_arg (), build_int_cst (integer_type_node, SFPLOADI_MOD0_USHORT));
