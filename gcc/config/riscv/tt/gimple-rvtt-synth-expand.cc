...
{
  gcc_assert (!(mod & SFPXLOADI_MOD0_32BIT_MASK));
  auto *new_insnd = rvtt_get_insn_data
    (mod & SFPXLOADI_MOD0_32BIT_MASK ? rvtt_insn_data::sfpxloadi : rvtt_insn_data::sfploadi);
		   ...
		   gimple_stmt_iterator *gsip, bool before)
{
  ...
  auto *new_insnd = rvtt_get_insn_data (rvtt_insn_data::sfploadi_lv);

  auto val = gimple_call_arg (stmt, insnd->nonimm_pos);
  gimple *shift_stmt = nullptr;
  if (TREE_CODE (val) == SSA_NAME)
    {
      tree tmp = make_ssa_name (TREE_TYPE (val));
      shift_stmt = gimple_build_assign
	(tmp, RSHIFT_EXPR, val,
	 build_int_cst (unsigned_type_node, 16));
      val = tmp;
    }
  else
    val = build_int_cst (TREE_TYPE (val), unsigned (TREE_INT_CST_LOW (val)) >> 16);
  ...
  gimple_call_set_arg (new_stmt, new_insnd->nonimm_pos, val);  // nonimm
  ...
  if (shift_stmt)
    {
      auto shift_gsi = gsi_for_stmt (new_stmt);
      finish_new_insn (&shift_gsi, true, shift_stmt, new_stmt);
    }
}
...
expand_complex (gcall *stmt, const rvtt_insn_data *insnd, gimple_stmt_iterator *gsip)
{
  switch (insnd->id)
      ...
      if ((get_int_arg (stmt, insnd->mod_pos) & SFPXLOADI_MOD0_32BIT_MASK) != 0)
	{
	  ...
	  gimple_call_set_fndecl (stmt, rvtt_get_insn_data (rvtt_insn_data::sfploadi)->decl);
