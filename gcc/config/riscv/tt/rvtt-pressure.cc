...
/* GIMPLE-side vector-register (LREG) pressure engine
   ...
   Stage-A conservatism contract: under flag_checking every public
   query recomputes its verdict with a verbatim copy of the retired
   mirror (the LEGACY section below) and asserts equality.  Any
   inequality is a live latent divergence, adjudicated as a finding.
   The legacy copies double as the one-pin rollback provision; delete
   them (and the asserts) next pin.  */
...
/* ====================================================================
   LEGACY MIRRORS (stage-A verdict identity + one-pin rollback).

   Verbatim copies of the three retired hand-kept counters, exactly as
   they stood at the pin-49 union in gimple-rvtt-prgm-const.cc,
   gimple-rvtt-reassoc.cc and gimple-rvtt-invariant.cc.  They no
   longer decide anything: every public query above recomputes its
   verdict through one of these under flag_checking and asserts
   equality (the item-#10 stage-A contract).  Delete this whole
   section, and the flag_checking blocks that call it, next pin.
   ==================================================================== */
/* A vector SSA value that will occupy an allocatable LREG.  Constant
   register reads (rvtt_sfpreadlreg with index >= SFPU_CREG_IDX_LWM)
   expand to a zero-cost cstlreg unspec folded into consumers
...
static bool
legacy_pressure_tracked_p (tree name)
{
  if (TREE_CODE (name) != SSA_NAME || !VECTOR_TYPE_P (TREE_TYPE (name)))
    return false;
  gimple *def = SSA_NAME_DEF_STMT (name);
...
static void
legacy_compute_lreg_pressure (function *fn, unsigned capacity,
			      rvtt_pressure_model *m)
{
  bitmap_obstack_initialize (&m->obstack);
  unsigned nbb = last_basic_block_for_fn (fn);
  ...
  while (changed)
      ...
      FOR_EACH_BB_FN (bb, fn)
	{
	  ...
	  FOR_EACH_EDGE (e, ei, bb->succs)
		...
		{
		  tree arg = gimple_phi_arg_def (psi.phi (), e->dest_idx);
		  if (legacy_pressure_tracked_p (arg))
		    changed |= bitmap_set_bit (out, SSA_NAME_VERSION (arg));
		}
	    }
	  ...
	  for (gimple_stmt_iterator gsi = gsi_last_bb (bb); !gsi_end_p (gsi);
		...
		continue;
	      tree lhs = gimple_get_lhs (stmt);
	      if (lhs && legacy_pressure_tracked_p (lhs))
		bitmap_clear_bit (in, SSA_NAME_VERSION (lhs));
	      ssa_op_iter iter;
	      tree use;
	      FOR_EACH_SSA_TREE_OPERAND (use, stmt, iter, SSA_OP_USE)
		if (legacy_pressure_tracked_p (use))
		  bitmap_set_bit (in, SSA_NAME_VERSION (use));
	    }
	  for (gphi_iterator psi = gsi_start_phis (bb); !gsi_end_p (psi);
	       gsi_next (&psi))
	    {
	      tree res = gimple_phi_result (psi.phi ());
	      if (legacy_pressure_tracked_p (res))
		bitmap_clear_bit (in, SSA_NAME_VERSION (res));
	    }
	  changed |= bitmap_ior_into (m->live_in[bb->index], in);
	  BITMAP_FREE (in);
  ...
  FOR_EACH_BB_FN (bb, fn)
	...
	unsigned v;
	EXECUTE_IF_SET_IN_BITMAP (live, 0, v, bi)
	  count += legacy_lreg_width (ssa_name (v));
      }
      unsigned bb_max = count;
      for (gimple_stmt_iterator gsi = gsi_last_bb (bb); !gsi_end_p (gsi);
	   gsi_prev (&gsi))
	{
	  ...
	  if (lhs && legacy_pressure_tracked_p (lhs))
	    {
	      if (bitmap_clear_bit (live, SSA_NAME_VERSION (lhs)))
		count -= legacy_lreg_width (lhs);
	      else
		/* Dead def: transiently occupies its registers here.  */
		bb_max = MAX (bb_max, count + legacy_lreg_width (lhs));
	    }
	  ssa_op_iter iter;
	  tree use;
	  FOR_EACH_SSA_TREE_OPERAND (use, stmt, iter, SSA_OP_USE)
...
static unsigned
legacy_bb_vec_pressure_peak (basic_block bb)
{
  if (!dom_info_available_p (CDI_DOMINATORS))
    calculate_dominance_info (CDI_DOMINATORS);
...
/* Keep the transformed loop within the architectural eight-LREG file before
   ...
   statement placement when the bound is exceeded.  */
static bool
legacy_loop_lreg_pressure_legal_p (class loop *loop,
				 const auto_vec<gcall *> &loads,
				 bool report, bool cc_transients,
				 bool exempt_creg_reads)
{
...
/* ====================================================================
   Public queries.  Each one answers from the engine and, under
   flag_checking, recomputes through the corresponding legacy mirror
   and asserts verdict identity.
   ==================================================================== */
		       ...
		       rvtt_pressure_model *m)
{
  ...
  if (flag_checking)
    {
      rvtt_pressure_model chk;
      legacy_compute_lreg_pressure (fn, capacity, &chk);
      gcc_assert (chk.peak == m->peak);
      gcc_assert (bitmap_equal_p (chk.over_bbs, m->over_bbs));
      gcc_assert (chk.live_in.length () == m->live_in.length ());
      for (unsigned ix = 0; ix < chk.live_in.length (); ++ix)
	{
	  gcc_assert (!chk.live_in[ix] == !m->live_in[ix]);
	  if (chk.live_in[ix])
	    gcc_assert (bitmap_equal_p (chk.live_in[ix], m->live_in[ix]));
	}
    }
}
...
{
  unsigned peak = engine_bb_peak (bb);
  if (flag_checking)
    gcc_assert (peak == legacy_bb_vec_pressure_peak (bb));
  return peak;
}
  ...
  if (flag_checking)
    {
	 ...
	 the whole testsuite's -fchecking vehicles police the table.  */
      rvtt_pressure_validate_lut_slot_facts ();
      gcc_assert (ok == legacy_loop_lreg_pressure_legal_p
			  (loop, loads, /*report=*/false, cc_transients,
			   exempt_creg_reads));
    }
  return ok;
}
