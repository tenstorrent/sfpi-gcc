...
void swap () {
  auto x = __builtin_rvtt_sfpreadlreg(9);
  auto y = __builtin_rvtt_wh_sfpload (nullptr, 0, 0, 0, 0, 0);
  __builtin_rvtt_wh_sfpswap (x, y, 0);
  auto z = __builtin_rvtt_wh_sfpmul (x, y, 0);
}
/*
**_ZN3cst4swapEv:
**	SFPLOAD	L0, 0, 0, 0
...
void foo () {
  auto x =  __builtin_rvtt_sfpreadlreg(9);
  auto f = __builtin_rvtt_wh_sfpload_lv (nullptr, x, 0, 0, 0, 0, 0);
  auto y = __builtin_rvtt_wh_sfpmul (f, x, 0);
}
/*
**_ZN2lv3fooEv:
...
void bar (int i) {
  ...
  if (i) {
    auto g = __builtin_rvtt_wh_sfpload (nullptr, 0, 0, 0, 0, 0);
    __builtin_rvtt_wh_sfpmul_lv (x, g, g, 0);
  }
  auto f = __builtin_rvtt_wh_sfpload (nullptr, 0, 0, 0, 0, 0);
  auto y = __builtin_rvtt_wh_sfpmul_lv (x, f, x, 0);
}
/*
**_ZN2lv3barEi:
**	beq	a0,zero,.L[0-9]+
