...
// Near miss: a leaf value that is also consumed outside its dispatch
// leaf cannot be dissolved into the LUT.
// { dg-final { scan-tree-dump "refused \\(lut-leaf-value-escapes\\)" "rvtt_lut_select" } }
...
lut_tree_leaf_escapes ()
{
  for (int ix = 0; ix < 8; ++ix)
    {
      sfpi::vFloat x = sfpi::dst_reg[0];
      sfpi::vFloat mag = sfpi::abs (x);
      sfpi::vFloat t = mag * 0.1875f + 0.3125f;
