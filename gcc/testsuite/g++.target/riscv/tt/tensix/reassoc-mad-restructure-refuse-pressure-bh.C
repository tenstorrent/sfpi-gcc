...
// Pressure near-miss (the item-#10 engine budget): five extra vector
// values live across the pair take the conservative block peak to the
// 8-LREG file, so keeping the loadi live to the mad has no headroom --
// the candidate refuses BY NAME (reassoc-pressure-budget-exceeded),
// the immediate fold proceeds, and the kernel keeps compiling exactly
// as before (a licensed transform must never make a compilable kernel
// uncompilable).
