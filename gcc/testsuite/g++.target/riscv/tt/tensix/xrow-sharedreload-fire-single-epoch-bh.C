// Single-epoch generality fire: the tanh anatomy with only ONE in-loop
// reload group (no second epoch, no next-definition WAR to
// manufacture) still dedupes -- the copy half's two-word fp32 group is
// deleted and its consumer reads the first half's materialization.
// Same 8/8-pressure starvation as the composed fire twin: three CReg
// residents (hoisted-reuse), one local park, and the copy's other
// three webs consume the free LREGs, pinning the reload web onto the
// shared register.
...
// { dg-final { scan-rtl-dump {Crossrow shared-reload: reg \d+ epochs=1 removed=2 II \d+ -> \d+} "rvtt_schedule" } }
// { dg-final { scan-rtl-dump {Crossrow pairing: bb \d+ rows=2 nodes=22} "rvtt_schedule" } }
