...
// stays intact and the final use observes the value that allocation kept in
// L1 across all eight rows.
// { dg-final { scan-assembler "SFPSTORE\tL1, 300, 0, 3" } }
...
#define MINMAX_ROW()                                                          \
  do                                                                          \
    {                                                                         \
      ...
      auto b = __builtin_rvtt_sfpload (nullptr, 64, 0, 0, 0, 3);              \
      auto pair = __builtin_rvtt_sfpswap (a, b, 1);                           \
      auto result = __builtin_rvtt_sfpselect2 (pair, 0);                      \
      __builtin_rvtt_sfpstore (nullptr, result, 300, 0, 0, 0, 3);             \
...
{
  auto saved = __builtin_rvtt_sfpload (nullptr, 200, 0, 0, 0, 3);
  MINMAX_ROW ();
  MINMAX_ROW ();
  MINMAX_ROW ();
  MINMAX_ROW ();
  ...
  MINMAX_ROW ();
  MINMAX_ROW ();
  __builtin_rvtt_sfpstore (nullptr, saved, 300, 0, 0, 0, 3);
}
