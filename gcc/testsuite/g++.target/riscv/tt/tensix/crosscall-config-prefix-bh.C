...
// Config-prefix pair FIRE: the callee's vConstFloatPrgm-idiom pair
// (all-constant sfploadi -> sfpwriteconfig_v to register 12) joins the
// coefficient contract -- re-materialized in the caller's loop
// preheader ahead of the contract loads, deleted from the callee.
// { dg-final { scan-tree-dump "config pair .creg 12. joins the contract" "rvtt_crosscall" } }
// { dg-final { scan-tree-dump "placed config pair .creg 12." "rvtt_crosscall" } }
// { dg-final { scan-tree-dump "config pair .creg 12. removed from" "rvtt_crosscall" } }
