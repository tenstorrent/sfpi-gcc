// Involution refusals: (a) no all-lanes proof at the first frame
// (crosslane-lane-state-unproven); (b) a companion-bank read outside
// any recognized frame (crosslane-companion-escape) -- deleting a
// transpose would change what that read observes.
...
// { dg-final { scan-tree-dump-not "involution cancel" "rvtt_crosslane" } }
