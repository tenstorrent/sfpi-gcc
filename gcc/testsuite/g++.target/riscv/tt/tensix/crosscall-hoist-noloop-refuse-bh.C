...
// Refusal twin: a second caller invokes the callee outside any loop --
// there is nothing to amortize against and the all-or-nothing caller
// proof refuses the whole contract by name.
// { dg-final { scan-tree-dump "refused .crosscall-caller-no-loop." "rvtt_crosscall" } }
// { dg-final { scan-tree-dump-not "hoisted" "rvtt_crosscall" } }

#define CCH_CALLEE cch_noloop_callee
#define CCH_CALLER cch_noloop_caller
#define CCH_AFTER_DEFS void cch_noloop_extra_caller () { cch_noloop_callee (); }
