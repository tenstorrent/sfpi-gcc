...
// A MOP launch in the tile loop defers to the TU-wide template census:
// with every programmed slot in an audited word class (NOP, SETRWC,
// ELWADD) and no SFPLOADI destination in the allocatable mask, the
// hoist fires across the launching loop.
...
static inline void xlh_mop_program_template (unsigned outer, unsigned inner)
{
...
xlh_mop_kernel (int tiles)
{
  xlh_mop_program_template (4, 2);
...
// entry.
extern "C" void _start () { xlh_mop_kernel (4); }
