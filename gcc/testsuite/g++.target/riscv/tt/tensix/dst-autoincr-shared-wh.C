...
// Wormhole: two 5-row groups pay for the per-execution program cost
// (3*2 config-class slots + 2 entry residual = 8) only through the shared
// dominating program (10 > 8).  The four-word rows
// exceed the replay minimum on this target, so the shared groups are
// launch-shaped: one executing capture plus nine playbacks.
