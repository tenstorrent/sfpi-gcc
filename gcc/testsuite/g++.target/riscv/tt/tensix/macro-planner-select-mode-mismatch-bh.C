...
// { dg-final { scan-rtl-dump "cc-template-unproved" "rvtt_macro_planner" } }
...
#define ROW()                                                                 \
  do                                                                          \
    {                                                                         \
      ...
      auto on_true = __builtin_rvtt_sfpload (nullptr, 32, 0, 0, 2, 7);        \
...
__attribute__((noinline)) void mode_mismatch ()
{
  ROW ();
  ROW ();
  ROW ();
