// Default-off proof: without -mtt-tensix-optimize-ims the fire
// kernel's self-loop keeps the plain list scheduler's original
// deferral wording -- the IMS machinery is invisible at every default.
// Same kernel as the fire twin.
// (The scan-not keys on the machinery's "(ims" spelling because this
// file's own NAME appears in the dump's source-location strings.)
...
// { dg-final { scan-rtl-dump-not "\\(ims" "rvtt_schedule" } }

void kd_ims_fire_off ()
{
  for (int d = 0; d < 32; ++d)
    {
