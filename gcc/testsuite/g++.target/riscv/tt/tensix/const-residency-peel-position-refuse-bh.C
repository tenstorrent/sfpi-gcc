// Peel-class near miss: the invariant materialization sits AFTER the
// body's first CC writer, i.e. inside/behind the v_if region, where
// iterations 2..N execute it under the region's PARTIAL lane state --
// not the all-lanes state the trailing SFPENCC re-establishes.  It is
// not a candidate; nothing is programmed and nothing is peeled.
