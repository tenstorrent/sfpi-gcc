// Wormhole twin of lreg-coalesce-fire-bh.C (WH no-increment address
// mode is 3; BH is 7).  The
...
// the absolute counts.
// { dg-options "-mcpu=tt-wh-tensix -O2 -mtt-tensix-optimize-lreg-alloc -mtt-tensix-optimize-lreg-coalesce -mtt-tensix-dst-layout-32b -fdump-rtl-rvtt_lp_alloc-details -fdump-rtl-rvtt_dst_ownership" }
...
#define ST(x, r) __builtin_rvtt_sfpstore (nullptr, (x), (r), 0, 0, 4, 3)

void coalesce_fire_wh (void)
{
  /* Fold source + identity reload: dst-ownership replaces the reload
     with the plain copy w = v.  */
  auto v  = __builtin_rvtt_sfpload (nullptr, 320, 0, 0, 4, 3);
  auto s1 = __builtin_rvtt_sfpxor (v, v);
  auto w  = __builtin_rvtt_sfpload (nullptr, 320, 0, 0, 4, 3);
