...
// Named refusal: a runtime trip count is not provably constant; the
// record-hoist never speculates the preheader record (a zero-trip entry
// would still record, and profitability needs proven trips).  The shape
// keeps the in-body record byte-identically.
// { dg-final { scan-rtl-dump "record-hoist refused: record-hoist-trip-count-unproven" "rvtt_replay" } }
