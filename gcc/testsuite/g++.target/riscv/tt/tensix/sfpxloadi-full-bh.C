...
// no temporary-to-result SFPMOV between the two halves of a 32-bit load.
// Keeping all results live until the stores also exercises LREG pressure.
// Verified 2026-09-25 against both designs -- ours (single builtin held to
// RTL) and main's (USHORT+UPPER split in immvar) -- which emit an identical
// 5x SFPLOADI / 0x SFPMOV sequence here.  The requirement is what matters;
// either implementation may satisfy it.
