...
// separator value.  The mechanism keys on structure only, never on a
// register calendar or constant fingerprint.
...
void rerecord_fire_renamed (volatile int *sink)
{
  ...
  for (unsigned k = 6; k != 0; --k)
    {
      ...
      *sink = (int) (k * 7u);	// volatile scalar separator
      p = __builtin_rvtt_sfpmul (p, q, 0);
      q = __builtin_rvtt_sfpmul (q, r, 0);
      r = __builtin_rvtt_sfpmul (r, r, 0);
      q = __builtin_rvtt_sfpmul (q, q, 0);
